...
bot.on('callback_query', async (query) => {
  ...
  // --- VILOYAT O'CHIRISH ---
  if (cData.startsWith('dv_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    delete db.hostel_structure[vil]; saveDB();
    bot.editMessageText(`🗑 <b>${vil}</b> viloyati va barcha maʼlumotlari tozalab tashlandi!`, { chat_id: chatId, message_id: query.message.message_id, parse_mode: 'HTML' });
    return bot.answerCallbackQuery(query.id, { text: "Viloyat tozalab tashlandi!" });
  }
  
  // --- FILIAL QO'SHISH ---
  else if (cData.startsWith('af_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    sessions[chatId].tempStruct.vil = sessions[chatId].tempOptions[idx];
    pushState(chatId, 'STRUCT_ADD_FILIAL_NAME');
    bot.editMessageText("Iltimos Kiritmoqchi boʻlgan Filiall nomini chatga yozing:", { chat_id: chatId, message_id: query.message.message_id });
    return bot.answerCallbackQuery(query.id);
  }
  
  // --- FILIAL O'CHIRISH ---
  else if (cData.startsWith('df_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.vil = vil;
    const filials = Object.keys(db.hostel_structure[vil] || {});
    sessions[chatId].tempOptions = filials;
    const kbd = { inline_keyboard: filials.map((f, i) => [{ text: f, callback_data: `df_f_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Filiallini tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('df_f_')) {
    const idx = parseInt(cData.split('_')[2]);
    const fil = sessions[chatId].tempOptions[idx];
    delete db.hostel_structure[sessions[chatId].tempStruct.vil][fil]; saveDB();
    bot.editMessageText(`🗑 <b>${fil}</b> filiali tozalab tashlandi!`, { chat_id: chatId, message_id: query.message.message_id, parse_mode: 'HTML' });
    return bot.answerCallbackQuery(query.id, { text: "Filial o'chirildi!" });
  }

  // --- XONA QO'SHISH ---
  else if (cData.startsWith('ax_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.vil = vil;
    const filials = Object.keys(db.hostel_structure[vil] || {});
    sessions[chatId].tempOptions = filials;
    const kbd = { inline_keyboard: filials.map((f, i) => [{ text: f, callback_data: `ax_f_${i}` }]) };
    bot.editMessageText("Xona kiritmoqchi boʻlgan Filialllingizdi tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('ax_f_')) {
    const idx = parseInt(cData.split('_')[2]);
    sessions[chatId].tempStruct.fil = sessions[chatId].tempOptions[idx];
    pushState(chatId, 'STRUCT_ADD_XONA_NAME');
    bot.editMessageText("Iltimos Kiritmoqchi boʻlgan Xona nomini chatga yozing:", { chat_id: chatId, message_id: query.message.message_id });
    return bot.answerCallbackQuery(query.id);
  }

  // --- XONA O'CHIRISH ---
  else if (cData.startsWith('dx_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.vil = vil;
    const filials = Object.keys(db.hostel_structure[vil] || {});
    sessions[chatId].tempOptions = filials;
    const kbd = { inline_keyboard: filials.map((f, i) => [{ text: f, callback_data: `dx_f_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Xonaning Filiallini tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('dx_f_')) {
    const idx = parseInt(cData.split('_')[2]);
    const fil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.fil = fil;
    const xonalar = Object.keys(db.hostel_structure[sessions[chatId].tempStruct.vil][fil] || {});
    sessions[chatId].tempOptions = xonalar;
    const kbd = { inline_keyboard: xonalar.map((x, i) => [{ text: x, callback_data: `dx_x_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Xonani tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('dx_x_')) {
    const idx = parseInt(cData.split('_')[2]);
    const xon = sessions[chatId].tempOptions[idx];
    delete db.hostel_structure[sessions[chatId].tempStruct.vil][sessions[chatId].tempStruct.fil][xon]; saveDB();
    bot.editMessageText(`🗑 <b>${xon}</b> xonasi tozalab tashlandi!`, { chat_id: chatId, message_id: query.message.message_id, parse_mode: 'HTML' });
    return bot.answerCallbackQuery(query.id, { text: "Xona o'chirildi!" });
  }

  // --- YOTOQ QO'SHISH ---
  else if (cData.startsWith('ay_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.vil = vil;
    const filials = Object.keys(db.hostel_structure[vil] || {});
    sessions[chatId].tempOptions = filials;
    const kbd = { inline_keyboard: filials.map((f, i) => [{ text: f, callback_data: `ay_f_${i}` }]) };
    bot.editMessageText("Yotoq kiritmoqchi boʻlgan Filialllingizdi tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('ay_f_')) {
    const idx = parseInt(cData.split('_')[2]);
    const fil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.fil = fil;
    const xonalar = Object.keys(db.hostel_structure[sessions[chatId].tempStruct.vil][fil] || {});
    sessions[chatId].tempOptions = xonalar;
    const kbd = { inline_keyboard: xonalar.map((x, i) => [{ text: x, callback_data: `ay_x_${i}` }]) };
    bot.editMessageText("Yotoq kiritmoqchi boʻlgan Xonani tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('ay_x_')) {
    const idx = parseInt(cData.split('_')[2]);
    sessions[chatId].tempStruct.xon = sessions[chatId].tempOptions[idx];
    pushState(chatId, 'STRUCT_ADD_YOTOQ_NAME');
    bot.editMessageText("Iltimos Kiritmoqchi boʻlgan Yotoq nomini chatga yozing:", { chat_id: chatId, message_id: query.message.message_id });
    return bot.answerCallbackQuery(query.id);
  }

  // --- YOTOQ O'CHIRISH ---
  else if (cData.startsWith('dy_v_')) {
    const idx = parseInt(cData.split('_')[2]);
    const vil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.vil = vil;
    const filials = Object.keys(db.hostel_structure[vil] || {});
    sessions[chatId].tempOptions = filials;
    const kbd = { inline_keyboard: filials.map((f, i) => [{ text: f, callback_data: `dy_f_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Yotoqning Filiallini tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('dy_f_')) {
    const idx = parseInt(cData.split('_')[2]);
    const fil = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.fil = fil;
    const xonalar = Object.keys(db.hostel_structure[sessions[chatId].tempStruct.vil][fil] || {});
    sessions[chatId].tempOptions = xonalar;
    const kbd = { inline_keyboard: xonalar.map((x, i) => [{ text: x, callback_data: `dy_x_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Yotoq Xonasini tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('dy_x_')) {
    const idx = parseInt(cData.split('_')[2]);
    const xon = sessions[chatId].tempOptions[idx];
    sessions[chatId].tempStruct.xon = xon;
    const yotoqlar = Object.keys(db.hostel_structure[sessions[chatId].tempStruct.vil][sessions[chatId].tempStruct.fil][xon] || {});
    sessions[chatId].tempOptions = yotoqlar;
    const kbd = { inline_keyboard: yotoqlar.map((y, i) => [{ text: y, callback_data: `dy_y_${i}` }]) };
    bot.editMessageText("Oʻchirish kerak boʻlgan Yotoqni tanlang:", { chat_id: chatId, message_id: query.message.message_id, reply_markup: kbd });
    return bot.answerCallbackQuery(query.id);
  }
  else if (cData.startsWith('dy_y_')) {
    const idx = parseInt(cData.split('_')[2]);
    const yot = sessions[chatId].tempOptions[idx];
    const st = sessions[chatId].tempStruct;
    const bedObj = db.hostel_structure[st.vil][st.fil][st.xon][yot];
    if (!bedObj.isFree) {
       return bot.answerCallbackQuery(query.id, { text: "⚠️ Rad etildi! Ushbu yotoqda hozirda Kvartirant yashamoqda.", show_alert: true });
    } else {
       delete db.hostel_structure[st.vil][st.fil][st.xon][yot]; saveDB();
