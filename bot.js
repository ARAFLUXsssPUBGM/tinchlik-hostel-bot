...
bot.on('message', async (msg) => {
  ...
  if (state.startsWith('REG_')) {
    ...
    if (state === 'REG_FISH') {
      ...
      if (db.hostel_structure[vil] && db.hostel_structure[vil][fil] && db.hostel_structure[vil][fil][text]) {
        ...
        // Dastur bo'sh yotoqlarni filtrlash qismi (Keyboard ko'rinishida chiqaradi)
  ...
  if (db.admins.includes(chatId) && state.startsWith('ADMIN_')) {
    ...
    if (state === 'ADMIN_MAIN') {
      ...
      // INLINE TUZILMA KELISHI
      else if (text === "🏨 HOSTEL Sozlash") {
        pushState(chatId, 'ADMIN_HOSTEL_STRUCT'); // Garchi Endi Inline bo'lsa ham state'ni o'rnatamiz
        const inlineKbd = {
          inline_keyboard: [
            [{ text: "➕ Viloyat qo'shish", callback_data: "menu_add_vil" }, { text: "🗑 Viloyatni o'chirish", callback_data: "menu_del_vil" }],
            [{ text: "➕ Filial qo'shish", callback_data: "menu_add_fil" }, { text: "🗑 Filialni o'chirish", callback_data: "menu_del_fil" }],
            [{ text: "➕ Xona qo'shish", callback_data: "menu_add_xon" }, { text: "🗑 Xonani o'chirish", callback_data: "menu_del_xon" }],
            [{ text: "➕ Yotoq va Narx qo'shish", callback_data: "menu_add_yot" }, { text: "🗑 Yotoqni o'chirish", callback_data: "menu_del_yot" }]
          ]
        };
        await clearAndSend(chatId, "🏨 <b>HOSTEL Struktura Matrixi:</b>\nAmallardan birini tanlang (Faqat tugmalardan foydalaning):", inlineKbd);
      }
      return;
              }
    ...
    if (state === 'ADMIN_SET_RULES') {
      ...
      // ========================================================================
    //               INLINE TASDIQLASH MATN INPUTLARI
    // ========================================================================
      ...
      if (!sessions[chatId].tempStruct) sessions[chatId].tempStruct = {};
      sessions[chatId].tempStruct.newVil = regionName;
      ...
      const inlineKeyboard = {
        inline_keyboard: [
          [
            { text: "✅ Ha, qo'shilsin", callback_data: "conf_vil_yes" },
            { text: "❌ Bekor qilish", callback_data: "conf_cancel" }
          ]
        ]
      };
      ...
      const filName = text.trim();
      sessions[chatId].tempStruct.newFil = filName;
      sessions[chatId].state = 'STRUCT_CONFIRM_FILIAL';
      saveSessions();
      
      const inlineKeyboard = {
        inline_keyboard: [[ { text: "✅ Tasdiqlash", callback_data: "conf_fil_yes" }, { text: "❌ Bekor qilish", callback_data: "conf_cancel" } ]]
      };
      await bot.sendMessage(chatId, `❓ Filial qo'shishni tasdiqlaysizmi?\n\nViloyat: ${sessions[chatId].tempStruct.vil}\nFilial nomi: <b>${filName}</b>`, {
        reply_markup: inlineKeyboard, parse_mode: 'HTML'
      });
      return;
    }
      ...
      const xonName = text.trim();
      sessions[chatId].tempStruct.newXon = xonName;
      sessions[chatId].state = 'STRUCT_CONFIRM_XONA';
      saveSessions();
      
      const inlineKeyboard = {
        inline_keyboard: [[ { text: "✅ Tasdiqlash", callback_data: "conf_xon_yes" }, { text: "❌ Bekor qilish", callback_data: "conf_cancel" } ]]
      };
      await bot.sendMessage(chatId, `❓ Xona qo'shishni tasdiqlaysizmi?\n\nFilial: ${sessions[chatId].tempStruct.fil}\nXona nomi: <b>${xonName}</b>`, {
        reply_markup: inlineKeyboard, parse_mode: 'HTML'
      });
      return;
    }
      ...
      sessions[chatId].tempStruct.newYot = text.trim();
      ...
      if (priceNum > 0) {
        sessions[chatId].tempStruct.newPrice = priceNum;
        sessions[chatId].state = 'STRUCT_CONFIRM_YOTOQ';
        saveSessions();
        const inlineKeyboard = {
          inline_keyboard: [[ { text: "✅ Tasdiqlash", callback_data: "conf_yot_yes" }, { text: "❌ Bekor qilish", callback_data: "conf_cancel" } ]]
        };
        const t = sessions[chatId].tempStruct;
        await bot.sendMessage(chatId, `❓ Yotoq va narx biriktirishni tasdiqlaysizmi?\n\nManzil: <b>${t.vil} -> ${t.fil} -> ${t.xon}</b>\nYotoq: <b>${t.newYot}</b>\nOylik Narxi: <b>${formatMoney(priceNum)}</b>`, {
          reply_markup: inlineKeyboard, parse_mode: 'HTML'
        });
      } else {
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('menu_')) {
    ...
    if (data === 'menu_add_vil') {
      ...
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_dv_${v}` }]) };
      await clearAndSend(chatId, "O'chiriladigan viloyatni tanlang (Diqqat: Ichidagi barcha ob'ektlar o'chadi!):", kbd);
    } 
    else if (data === 'menu_add_fil') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Avval viloyat qo'shing!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_afv_${v}` }]) };
      await clearAndSend(chatId, "Filial qo'shish uchun viloyatni tanlang:", kbd);
    } 
    else if (data === 'menu_del_fil') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Viloyatlar mavjud emas!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_dfv_${v}` }]) };
      await clearAndSend(chatId, "Filial o'chirish uchun avval viloyatni tanlang:", kbd);
    } 
    else if (data === 'menu_add_xon') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Viloyat mavjud emas!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_axv_${v}` }]) };
      await clearAndSend(chatId, "Xona qo'shish uchun viloyatni tanlang:", kbd);
    } 
    else if (data === 'menu_del_xon') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Viloyat mavjud emas!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_dxv_${v}` }]) };
      await clearAndSend(chatId, "Xona o'chirish uchun viloyatni tanlang:", kbd);
    } 
    else if (data === 'menu_add_yot') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Viloyat mavjud emas!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_ayv_${v}` }]) };
      await clearAndSend(chatId, "Yotoq joyi qo'shish uchun viloyatni tanlang:", kbd);
    } 
    else if (data === 'menu_del_yot') {
      const keys = Object.keys(db.hostel_structure || {});
      if (keys.length === 0) return bot.sendMessage(chatId, "Viloyat mavjud emas!");
      const kbd = { inline_keyboard: keys.map(v => [{ text: v, callback_data: `cb_dyv_${v}` }]) };
      await clearAndSend(chatId, "Yotoq o'chirish uchun viloyatni tanlang:", kbd);
    }
    return;
  }

  // CASCADING INLINE SELECTIONS
  if (data.startsWith('cb_')) {
    try { await bot.deleteMessage(chatId, query.message.message_id); } catch(e){}
    const prefix = data.split('_')[1]; 
    const val = data.substring(data.indexOf('_', 3) + 1);

    // Delete Viloyat
    if (prefix === 'dv') {
      if (db.hostel_structure[val]) { delete db.hostel_structure[val]; saveDB(); }
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `🗑 Viloyat "${val}" va ichidagi barcha ob'ektlar o'chirildi.`, adminMainKeyboard);
    } 
    // Add Filial (Selected Viloyat)
    else if (prefix === 'afv') {
      sessions[chatId].tempStruct.vil = val;
      pushState(chatId, 'STRUCT_ADD_FILIAL_NAME');
      await clearAndSend(chatId, `"${val}" viloyati uchun yangi Filial nomini kiriting:`, backKeyboard);
    }
    // Delete Filial (Selected Viloyat) -> Show Filials
    else if (prefix === 'dfv') {
      sessions[chatId].tempStruct.vil = val;
      const filials = Object.keys(db.hostel_structure[val] || {});
      if (filials.length === 0) return clearAndSend(chatId, "Bazada o'chirish uchun filial yo'q!", adminMainKeyboard);
      const kbd = { inline_keyboard: filials.map(f => [{ text: f, callback_data: `cb_dff_${f}` }]) };
      await clearAndSend(chatId, "O'chiriladigan filialni belgilang:", kbd);
    }
    // Delete Filial (Selected Filial)
    else if (prefix === 'dff') {
      const vil = sessions[chatId].tempStruct.vil;
      if (db.hostel_structure[vil] && db.hostel_structure[vil][val]) { delete db.hostel_structure[vil][val]; saveDB(); }
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `🗑 Filial "${val}" to'liq tozalab tashlandi.`, adminMainKeyboard);
    }
    // Add Xona (Select Viloyat)
    else if (prefix === 'axv') {
      sessions[chatId].tempStruct.vil = val;
      const filials = Object.keys(db.hostel_structure[val] || {});
      const kbd = { inline_keyboard: filials.map(f => [{ text: f, callback_data: `cb_axf_${f}` }]) };
      await clearAndSend(chatId, "Filialni tanlang:", kbd);
    }
    // Add Xona (Select Filial)
    else if (prefix === 'axf') {
      sessions[chatId].tempStruct.fil = val;
    ...
    // Delete Xona (Select Viloyat)
    else if (prefix === 'dxv') {
      sessions[chatId].tempStruct.vil = val;
      const filials = Object.keys(db.hostel_structure[val] || {});
      const kbd = { inline_keyboard: filials.map(f => [{ text: f, callback_data: `cb_dxf_${f}` }]) };
      await clearAndSend(chatId, "Filialni tanlang:", kbd);
    }
    // Delete Xona (Select Filial)
    else if (prefix === 'dxf') {
      sessions[chatId].tempStruct.fil = val;
      const vil = sessions[chatId].tempStruct.vil;
      const xonalar = Object.keys(db.hostel_structure[vil][val] || {});
      const kbd = { inline_keyboard: xonalar.map(x => [{ text: x, callback_data: `cb_dxx_${x}` }]) };
      await clearAndSend(chatId, "O'chiriladigan xonani belgilang:", kbd);
    }
    // Delete Xona (Select Xona)
    else if (prefix === 'dxx') {
      const vil = sessions[chatId].tempStruct.vil;
      const fil = sessions[chatId].tempStruct.fil;
      if (db.hostel_structure[vil] && db.hostel_structure[vil][fil]) { delete db.hostel_structure[vil][fil][val]; saveDB(); }
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `🗑 Xona o'chirildi.`, adminMainKeyboard);
    }
    // Add Yotoq (Select Viloyat)
    else if (prefix === 'ayv') {
      sessions[chatId].tempStruct.vil = val;
      const filials = Object.keys(db.hostel_structure[val] || {});
      const kbd = { inline_keyboard: filials.map(f => [{ text: f, callback_data: `cb_ayf_${f}` }]) };
      await clearAndSend(chatId, "Filialni bosing:", kbd);
    }
        // Add Yotoq (Select Filial)
    else if (prefix === 'ayf') {
      sessions[chatId].tempStruct.fil = val;
      const vil = sessions[chatId].tempStruct.vil;
      const xonalar = Object.keys(db.hostel_structure[vil][val] || {});
      const kbd = { inline_keyboard: xonalar.map(x => [{ text: x, callback_data: `cb_ayx_${x}` }]) };
      await clearAndSend(chatId, "Xonani tanlang:", kbd);
    }
    // Add Yotoq (Select Xona)
    else if (prefix === 'ayx') {
      sessions[chatId].tempStruct.xon = val;
      pushState(chatId, 'STRUCT_ADD_YOTOQ_NAME');
      await clearAndSend(chatId, "Yotoq joy nomini yozing (Masalan: 1-yotoq (A)):", backKeyboard);
    }
    // Delete Yotoq (Select Viloyat)
    else if (prefix === 'dyv') {
      sessions[chatId].tempStruct.vil = val;
      const filials = Object.keys(db.hostel_structure[val] || {});
      const kbd = { inline_keyboard: filials.map(f => [{ text: f, callback_data: `cb_dyf_${f}` }]) };
      await clearAndSend(chatId, "Filialni belgilang:", kbd);
    }
    // Delete Yotoq (Select Filial)
    else if (prefix === 'dyf') {
      sessions[chatId].tempStruct.fil = val;
      const vil = sessions[chatId].tempStruct.vil;
      const xonalar = Object.keys(db.hostel_structure[vil][val] || {});
      const kbd = { inline_keyboard: xonalar.map(x => [{ text: x, callback_data: `cb_dyx_${x}` }]) };
      await clearAndSend(chatId, "Xonani belgilang:", kbd);
    }
    // Delete Yotoq (Select Xona)
    else if (prefix === 'dyx') {
      sessions[chatId].tempStruct.xon = val;
      const vil = sessions[chatId].tempStruct.vil;
      const fil = sessions[chatId].tempStruct.fil;
      const yotoqlar = Object.keys(db.hostel_structure[vil][fil][val] || {});
      const kbd = { inline_keyboard: yotoqlar.map(y => [{ text: y, callback_data: `cb_dyy_${y}` }]) };
      await clearAndSend(chatId, "O'chiriladigan yotoqni tanlang:", kbd);
    }
    // Delete Yotoq (Select Yotoq)
    else if (prefix === 'dyy') {
      const vil = sessions[chatId].tempStruct.vil;
      const fil = sessions[chatId].tempStruct.fil;
      ...
      if (db.hostel_structure[vil] && db.hostel_structure[vil][fil] && db.hostel_structure[vil][fil][xon]) { 
        delete db.hostel_structure[vil][fil][xon][val]; saveDB(); 
      }
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
  ...
  // --- STRUKTURA TASDIQLASH (CONFIRM) ---
  if (data.startsWith('conf_')) {
    try { await bot.deleteMessage(chatId, query.message.message_id); } catch(e){}
    const t = sessions[chatId]?.tempStruct;

    if (data === 'conf_cancel') {
      sessions[chatId].state = 'ADMIN_MAIN'; delete sessions[chatId].tempStruct; saveSessions();
      await clearAndSend(chatId, "❌ Jarayon bekor qilindi.", adminMainKeyboard);
    }
    else if (data === 'conf_vil_yes' && t && t.newVil) {
      if (!db.hostel_structure) db.hostel_structure = {};
      if (!db.hostel_structure[t.newVil]) db.hostel_structure[t.newVil] = {};
      saveDB();
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `✅ <b>Muvaffaqiyatli:</b> "${t.newVil}" viloyati tizimga qo'shildi!`, adminMainKeyboard);
    }
    else if (data === 'conf_fil_yes' && t && t.vil && t.newFil) {
      if (!db.hostel_structure[t.vil]) db.hostel_structure[t.vil] = {};
      if (!db.hostel_structure[t.vil][t.newFil]) db.hostel_structure[t.vil][t.newFil] = {};
      saveDB();
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `✅ Filial "${t.newFil}" muvaffaqiyatli yaratildi.`, adminMainKeyboard);
    }
    else if (data === 'conf_xon_yes' && t && t.vil && t.fil && t.newXon) {
      if (!db.hostel_structure[t.vil]) db.hostel_structure[t.vil] = {};
      if (!db.hostel_structure[t.vil][t.fil]) db.hostel_structure[t.vil][t.fil] = {};
      if (!db.hostel_structure[t.vil][t.fil][t.newXon]) db.hostel_structure[t.vil][t.fil][t.newXon] = {};
      saveDB();
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `✅ Xona "${t.newXon}" omborga qo'shildi.`, adminMainKeyboard);
    }
    else if (data === 'conf_yot_yes' && t && t.vil && t.fil && t.xon && t.newYot && t.newPrice) {
      if (!db.hostel_structure[t.vil]) db.hostel_structure[t.vil] = {};
      if (!db.hostel_structure[t.vil][t.fil]) db.hostel_structure[t.vil][t.fil] = {};
      if (!db.hostel_structure[t.vil][t.fil][t.xon]) db.hostel_structure[t.vil][t.fil][t.xon] = {};
      
      db.hostel_structure[t.vil][t.fil][t.xon][t.newYot] = {
        price: t.newPrice,
        isFree: true
      };
      saveDB();
      sessions[chatId].state = 'ADMIN_MAIN'; saveSessions();
      await clearAndSend(chatId, `✅ Joy va uning narxi mukammal o'rnatildi!\nNarxi: <b>${formatMoney(t.newPrice)}</b>`, adminMainKeyboard);
    }
    return;
  }

  // --- ARIZALAR/TO'LOVLAR TASDIQLASH (QOLGAN QISMI SAQLAB QOLINDI) ---                               
...
async function handleStateReturn(chatId, prevState) {
  if (prevState === 'MAIN_MENU') {
    ...
    // Arxitektura ustuni inline holatga o'tkazilganda
    const inlineKbd = {
      inline_keyboard: [
        [{ text: "➕ Viloyat qo'shish", callback_data: "menu_add_vil" }, { text: "🗑 Viloyatni o'chirish", callback_data: "menu_del_vil" }],
        [{ text: "➕ Filial qo'shish", callback_data: "menu_add_fil" }, { text: "🗑 Filialni o'chirish", callback_data: "menu_del_fil" }],
    ...
    await clearAndSend(chatId, "🏨 <b>HOSTEL Struktura Matrixi:</b>\nAmallardan birini tanlang:", inlineKbd);
