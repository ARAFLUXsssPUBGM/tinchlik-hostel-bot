...
module.exports = async (msg, state) => {
  ...
  if (state === 'MAIN_MENU') {
    if (text === "👤 Roʻyxatdan oʻtish") { pushState(chatId, 'REG_FISH'); return clearAndSend(chatId, "1. To'liq F.I.SH (Familiya, Ism, Sharifingiz) ni kiriting:", backKeyboard); }
  ...
  if (state === 'REG_GENDER' && (text === "Erkak" || text === "Ayol")) {
    ...
    if (!vils.length) { sessions[chatId].state = 'MAIN_MENU'; return clearAndSend(chatId, "Hozircha tizimga viloyatlar kiritilmagan.", mainKeyboard); }
    const kbd = { keyboard: vils.map(v => [{text: v}]), resize_keyboard: true }; kbd.keyboard.push([{text: "⬅️ Ortga qaytish"}]);
    return clearAndSend(chatId, "7. O'zingizga qulay viloyatni tanlang:", kbd);
  }
};
